...
@section('content')
    <div class="max-w-5xl mx-auto">
        <div class="bg-white rounded-[24px] p-[32px]">
            <h2 class="py-[32px] px-[28px] text-[24px] font-semibold text-gray-800">Edit</h2>

            <form action="{{ route('alasan.update', $alasan->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-6 mb-6">
                    <div class="w-1/2">
                        ...
                        <select name="service_id" id="service_id" required
                            ...
                            @foreach ($services as $service)
                                <option value="{{ $service->id }}" {{ $alasan->service_id == $service->id ? 'selected' : '' }} class="px-4">
                                    {{ $service->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-1/2">
                        ...
                        <input type="text" name="title" id="title" required
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring focus:ring-blue-200 focus:outline-none"
                            placeholder="Enter Title" value="{{ old('title', $alasan->title) }}">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div id="upload-container" class="flex flex-col">
                        ...
                        <div id="current-image-container" class="flex flex-col items-center gap-1 mt-2">
                            ...
                            <img src="{{ asset('storage/' . $alasan->image) }}" alt="Current Image"
                ...
                <div class="mb-10">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring focus:ring-blue-200 focus:outline-none"
                        placeholder="Enter description">{{ old('description', $alasan->description) }}</textarea>
                </div>
