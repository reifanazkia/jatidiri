...
@section('page_title', 'Add Bonus')

@section('content')
    <div class="max-w-full mx-auto">
        <div class="bg-white rounded-[24px] p-[32px]">
            <h2 class="mb-4 text-[24px] font-semibold text-gray-800">Add Bonus</h2>
            ...
            <form action="{{ route('bonus.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex gap-6 mb-6">
                    <div class="w-full md:w-1/2 mb-4">
                        <label for="service_id" class="block text-sm font-medium text-gray-700 mb-1">Services <span
                            ...
                            <select name="service_id" id="service_id" required
                                ...
                                @foreach ($services as $service)
                                    <option value="{{ $service->id }}"
                                        {{ old('service_id') == $service->id ? 'selected' : '' }} class="py-2">
                                        {{ $service->name }}</option>
                                @endforeach
                            </select>
                            <div
                ...
                <!-- Image Upload -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Image <span
                            class="text-red-500">*</span></label>
                    <div id="upload-container" class="flex flex-col">
                        ...
                        <button type="button" onclick="document.getElementById('image').click()"
                            class="w-full px-4 py-2 mb-[32px] bg-white border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-4">
                            <span
                            ...
                            <span id="file-name" class="text-sm text-gray-500">No file chosen (Max: 2MB,
    ...
    <script>
        document.getElementById('image').addEventListener('change', function(e) {
            ...
            document.getElementById('file-name').textContent = fileName + ' (Max: 2MB, JPG/JPEG/PNG/WEBP)';
        });
        ...
        function previewImage(input, previewId, uploadContainerId, previewContainerId) {
            ...
            if (file) {
                // Display file name
                fileNameDisplay.textContent = file.name + ' (Max: 2MB, JPG/JPEG/PNG/WEBP)';
                ...
                fileNameDisplay.textContent = 'No file chosen (Max: 2MB, JPG/JPEG/PNG/WEBP)';
        ...
        function removeImage(previewId, inputId, uploadContainerId, previewContainerId) {
            ...
            document.getElementById('file-name').textContent = 'No file chosen (Max: 2MB, JPG/JPEG/PNG/WEBP)';
