...
<div class="max-w-full mx-auto p-[32px] bg-white rounded-[24px] shadow-md">
    ...
    <form action="{{ route('legal.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description"
                class="w-full border border-gray-300 rounded-md px-4 py-2 min-h-[150px] focus:outline-none focus:ring focus:border-blue-500">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            <div>
                ...
                <label for="image"
                    class="mt-1 flex justify-center px-6 pt-5 pb-6 border border-gray-300 rounded-md cursor-pointer">
                    <div class="space-y-1 text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-7 h-7 mx-auto">
            ...
            <div>
                <img id="preview" src="#" alt="Image preview"
                    class="rounded-lg w-full h-[250px] object-cover hidden border border-gray-300" />
            </div>
        </div>
