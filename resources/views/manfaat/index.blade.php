...
@section('page_title', 'Manfaat services')
...
@section('content')
    <div class="max-w-full mx-auto">
        <div class="bg-white rounded-[24px] shadow-md p-6">
            ...
            <div class="flex gap-4 items-center mb-3 relative">
                ...
                <div class="w-[180px]">
                    <div class="relative">
                        <select name="manfaat" id="manfaat" required
                ...
                <div
                    ...
                    <a href="{{ route('manfaat.create') }}" id="addNewButton"
                        ...
                        @foreach ($data as $item)
                            <tr class="h-[36px]">
                                ...
                                <td class="px-2 py-2 flex border border-gray-300">
                                    <a href="{{ route('manfaat.edit', $item->id) }}" id="editButton-{{ $item->id }}"
                                    ...
                                    <form action="{{ route('manfaat.destroy', $item->id) }}" method="POST"
                                        class="inline delete-form" data-id="{{ $item->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
    ...
    <script>
        ...
        // Show success message if session has success status
        @if (session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Success!',
        ...
        // Show error message if session has error status
        @if (session('error'))
            Swal.fire({
                icon: 'error',
                title: 'Error!',
