...
@section('page_title', 'Edit Why Services')

@section('content')
    <div class="max-w-full mx-auto">
        <div class="bg-white rounded-[24px] p-[32px]">
            <h2 class="mb-4 text-[24px] font-semibold text-gray-800">Edit</h2>

            <form action="{{ route('manfaat.update', $manfaat->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-6 mb-6">
                    <div class="w-full md:w-1/2 mb-4">
                        ...
                        <div class="relative">
                            <select name="service_id" id="services" required
                                ...
                                <option value="" disabled class="text-gray-400">-- Select Service --</option>
                                @foreach ($services as $service)
                                    <option value="{{ $service->id }}"
                                        {{ $manfaat->service_id == $service->id ? 'selected' : '' }} class="py-2">
                                        {{ $service->name }}</option>
                                @endforeach
                            </select>
                            <div
                    ...
                    <div class="w-1/2">
                        ...
                        <input type="text" name="title" id="title" required
                            ...
                            placeholder="Enter Title" value="{{ old('title', $manfaat->title) }}">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div id="upload-container" class="space-y-2">
                        ...
                        <button type="button" onclick="document.getElementById('image').click()"
                            class="w-full px-4 py-2 bg-white border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center gap-3">
                            <span
                                class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-800 border text-xs font-medium rounded-md hover:bg-gray-200 transition-colors">
                                Pilih File
                            </span>
                            <span id="file-name" class="text-sm text-gray-500 truncate flex-1 text-left">
                                {{ $manfaat->image ? basename($manfaat->image) : 'No file chosen' }}
                            </span>
                        </button>

                        <!-- Preview container -->
                        <div id="preview-container"
                            class="{{ $manfaat->image ? 'flex' : 'hidden' }} flex-col mt-6 space-y-1">
                            @if ($manfaat->image)
                                <img id="preview_image" src="{{ asset('storage/' . $manfaat->image) }}"
                            ...
                            @else
                                <img id="preview_image" src="#" alt="Image Preview"
                                    class="hidden w-20 h-20 object-cover rounded-md border border-gray-200">
                            @endif
                ...
                <div class="mb-10 mt-6">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring focus:ring-blue-200 focus:outline-none"
                        placeholder="Enter description">{{ old('description', $manfaat->description) }}</textarea>
                </div>
